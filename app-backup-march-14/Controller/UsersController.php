...
class UsersController extends AppController
{
	...
	if ($this->request->is('post')) {
	    ...
	    //error_log($this->request->data);
                        ...
                        $this->request->data['User']['first_name'] = $this->request->data['first'];
                        $this->request->data['User']['last_name'] = $this->request->data['last'];
                        $this->request->data['User']['email'] = $this->request->data['email'];
                        $this->request->data['User']['password'] = "LKJslJfsadkljASdFu239skjh$1";
                        $this->request->data['User']['job_title'] = "Job Seeker";
                        $this->request->data['User']['group_id'] = 2; // Job seeker group id
                        $this->request->data['User']['user_group_id'] = 3; // Job seeker group id
	                $this->request->data['User']['role'] = 2; // staff role id
		...
		if ($userInfo) {
		    ...
		    //echo "User not found. Will create it";
                        ...
                        if ($UserValidate) {
                            ...
                            if ($this->User->save($this->request->data)) {
                                ...
                                if ($this->UserDetail->save($this->request->data, false)) {
                                    ...
                                    //return json success message
                                    //$response = array('bug' => 0, 'msg' => 'Successfully created new user');
                                    //return json_encode($response);
                                }
                            }
                        } else {
                            //return json failure message
                            $response = array('bug' => 1, 'msg' => 'invalid info received from google');
	    ...
	    if ($this->Auth->login()) {
		    ...
		    if ($groupId == 4) {
			//echo "Redirecting to deals page";
                        //echo Router::url(array('controller' => 'deals', 'action' => 'index'), true);
                        $response = array('redirect' => Router::url(array('controller' => 'deals', 'action' => 'index'), true));
                        return json_encode($response);
                        //return $this->redirect(array('controller' => 'deals', 'action' => 'index'));
		    } else {
			//echo "Redirecting to admin page";
                        //echo Router::url(array('controller' => 'admins', 'action' => 'index'), true);
                        $response = array('redirect' => Router::url(array('controller' => 'admins', 'action' => 'index'), true));
                        return json_encode($response);
    ...
    public function logout()
    {
        ...
        $this->redirect(array('controller' => 'landing', 'action' => 'landingPage'));
    }
