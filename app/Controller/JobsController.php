<?php
/**
 * Class for performing all job related functions
 * 
 */
...
class JobsController extends AppController
{
    ...
    public function index()
    {
        ...
        $conditions['Deal.status'] = array('0', '1');
        $options[] = array(
            'alias' => 'Pipeline',
            'table' => 'pipeline',
            'type' => 'LEFT',
    ...
    public function add()
    {
	    ...
	    if (is_string($this->request->data['Deal']['company_id']) && (strncmp($this->request->data['Deal']['company_id'], "NEWCOMPANY-", strlen("NEWCOMPANY-")) === 0)) {
		$this->request->data['Company']['name'] = substr($this->request->data['Deal']['company_id'], strlen("NEWCOMPANY-"));
		$this->request->data['Company']['email'] = "hello@example.com";
		$this->request->data['Company']['address'] = "";
		$this->request->data['Company']['city'] = "";
		$this->request->data['Company']['state'] = "";
		$this->request->data['Company']['zip_code'] = "";
		$this->request->data['Company']['country'] = "";
		$this->request->data['Company']['website'] = "";
		$this->request->data['Company']['skype'] = "";
		$this->request->data['Company']['facebook'] = "";
		$this->request->data['Company']['linkedIn'] = "";
		$this->request->data['Company']['twitter'] = "";
                $this->Company->create();
    
                //save new company
                if ($this->Company->save($this->request->data)) {
                    //get company id
		    $this->request->data['Deal']['company_id'] = $this->Company->getLastInsertId();
		}
	    }
            //save new deal
    ...
    /**
     * This function is used to display favorite deal page.
     *
     * @return array
     */
	...
	//echo '</pre>';
        ...
        if ($this->request->is('post')) {
            ...
            $this->request->data['Deal']['id'] = $this->request->data['Job']['id'];
            $this->request->data['Deal']['history'] = $this->request->data['Job']['history'];
            $this->request->data['Deal']['status'] = 1;
            $dealId = $this->request->data['Deal']['id'];
            //seve deal as won
            if ($this->Deal->save($this->request->data)) {
    ...
    public function cview($id = Null)
    {
        ...
        if (!$deal) {
            return $this->redirect(
                    array('controller' => 'jobs', 'action' => 'client')
            );
        }
